...
export class StepRetrievecardComponent implements OnInit {
    ...
    startBusiness() {
        const all = this.commonService.doOff(this.DEVICE_LIGHT_CODE_IC_READER).mergeMap(data =>
            this.commonService.doOff(this.DEVICE_LIGHT_CODE_OCR_READER)
        ).mergeMap(data => {
            return this.commonService.doFlash(this.readType === 1 ? this.DEVICE_LIGHT_CODE_IC_READER : this.DEVICE_LIGHT_CODE_OCR_READER).mergeMap(data => {
                return this.service.sendRequest(CHANNEL_ID_RR_CARDREADER, 'closecard').mergeMap(data => {
                    if (this.readType === 1) {
                        ...
                        return Observable.timer(2000).mergeMap(val => this.service.sendRequestWithLog(CHANNEL_ID_RR_CARDREADER, 'listcardreaderswithhkic'))
                        .map(data => {
        ...
        all.subscribe(data => {
            ...
            this.processPromt('SCN-GEN-STEPS.NOT-COLLECT-CARD');
        ...
        //         this.commonService.doFlash(this.DEVICE_LIGHT_CODE_OCR_READER).mergeMap((data) => {
        //             return Observable.merge(this.commonService.doOff(this.DEVICE_LIGHT_CODE_OCR_READER),
        //             this.commonService.doOff(this.DEVICE_LIGHT_CODE_IC_READER),
        //             this.commonService.doOff(this.DEVICE_LIGHT_ALERT_BAR_BLUE_CODE),
        //             this.commonService.doOff(this.DEVICE_LIGHT_ALERT_BAR_GREEN_CODE),
        //             this.commonService.doOff(this.DEVICE_LIGHT_ALERT_BAR_RED_CODE)
        //             ).delay(this.PAGE_COLLECT__RETURN_CARD_ITEMOUT);
        //         }).subscribe(data => {
        //             this.service.sendTrackLog('333333333333333333333333');
        //             this.timer.ngOnDestroy();
        //             this.commonService.doCloseWindow();
        //         });
